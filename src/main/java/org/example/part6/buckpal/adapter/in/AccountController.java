...
import org.example.part6.buckpal.adapter.in.AccountResource;
import org.example.part6.buckpal.application.port.in.CreateAccountCommand;
import org.example.part6.buckpal.application.port.in.CreateAccountUseCase;
import org.example.part6.buckpal.application.port.in.ListAccountQuery;
import org.example.part6.buckpal.application.port.in.LoadAccountQuery;
...
import org.example.part6.buckpal.application.port.in.SendMoneyUseCase;
...
class AccountController {
    ...
    @GetMapping("/accounts")
    List<org.example.part6.buckpal.adapter.in.AccountResource> listAccounts() {
        List<Account> accounts = listAcountsQuery.getAccounts();
        return accounts.stream()
                .map(a -> new org.example.part6.buckpal.adapter.in.AccountResource(a.getId(), a.getBaselineBalance(), a.getActivityWindow()))
                .toList();
    }

    @GetMapping("/accounts/{accountId}")
    org.example.part6.buckpal.adapter.in.AccountResource getAccount(@PathVariable("accountId") Long accountId) {
        Account account = loadAcountQuery.getAccount(new AccountId(accountId));
        return new org.example.part6.buckpal.adapter.in.AccountResource(
                account.getId(),
    ...
    org.example.part6.buckpal.adapter.in.AccountResource createAccount(@RequestBody org.example.part6.buckpal.adapter.in.AccountResource account) {
